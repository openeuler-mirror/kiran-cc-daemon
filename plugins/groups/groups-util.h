...
namespace Kiran
{
#define AUTH_GROUP_ADMIN "com.kylinsec.kiran.system-daemon.groups.group-administration"

#define SPAWN_DBUS_WITH_ARGS(invocation, argv)                                             \
    {                                                                                      \
        std::string error;                                                                 \
        if (!GroupsUtil::spawn_with_login_uid(invocation.getMessage(), argv, error))       \
        {                                                                                  \
