...
void TimedateManager::init_ntp_units()
{
    ...
    if (this->ntp_unit_name_.empty())
    {
        if (ntp_units.empty())
        {
            KLOG_WARNING_TIMEDATE("No available NTP units found, skip NTP initialization.");
            return;
        }
        this->ntp_unit_name_ = ntp_units.front();
    }

    KLOG_INFO_TIMEDATE("Use %s as default NTP service, other NTP service will be stopped.", this->ntp_unit_name_.c_str());
